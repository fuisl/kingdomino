...
as AI integration and multiplayer capabilities.

The paper is structured as follows: Section II describes the problem and
provides a formal description of the game mechanics. Section III reviews
related work and relevant algorithms. Section IV discusses the teamwork and
collaboration tools used. Section V outlines the proposed approaches, including
the event-driven architecture and input handling. Section VI details the
implementation, including game states, tile placement, and scoring mechanisms.
Section VII presents experimental results and performance analysis. Finally,
Section VIII concludes the paper and suggests future work.

The source code for the Kingdomino digital adaptation can be found on GitHub at
...
planning techniques. (Fig~\ref{fig:kanban}).
...
The \texttt{BoardInputController} class handles input from game controllers. It
captures axis movements and button presses, translating them into actions such
as moving, rotating, placing, or discarding a domino. The controller input is
processed with a dead zone to ignore small, unintended movements, ensuring
precise control.
...
The \texttt{BoardInputProcessor} class handles input from the keyboard. It maps
key presses to actions such as moving, rotating, placing, or discarding a
domino. This class ensures that keyboard inputs are correctly translated into
game actions.
...
responsible for translating the inputs received from the
\texttt{BoardInputController} and \texttt{BoardInputProcessor} into signals
that the system can process. These handlers manage the state of the game and
ensure that inputs are processed correctly based on the current game state.
