...
\begin{figure}[htbp]
    \centerline{\includegraphics[width=0.5\textwidth]{assets/Model.png}}
    \caption{Class diagram of Kingdomino implementation.}\label{fig:kingdomino-class}
\end{figure}

\subsection{Used Libraries and Environment}
% -- Mention languages, frameworks, libraries, operating systems, version details, etc.

We incoperated many different software and libraries in the creation of our game:

\begin{itemize}
    \item LibGDX as our game library.
    \item Textures are drawn using GNU Image Manipulation Program.
    \item Texture Atlas are generated with GDX Texture Packer.
    \item We used Gradle to manage the build and run process during development.
    \item Shader-based effects are written in OpenGL.
    \item Distributable version of our game are built by Construo, a Gradle plugin.
    \item Our distributable are built automatically every time there is a change in our codebase using Github Action.
\end{itemize}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \Require{Object, Duration, Magnitude}
        \Ensure{Object's position is shaken and reset}
